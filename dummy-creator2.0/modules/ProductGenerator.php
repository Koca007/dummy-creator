...
class ProductGenerator {
    ...
    protected $selected_attributes; // New property for storing selected attributes from the form
    ...
    public function __construct(
        ...
        $selected_attributes = [], // Selected attributes from the form
        $variation_images = '',
        ...
        $this->gallery_images = is_string($gallery_images) && !empty($gallery_images) ? array_map('trim', explode(',', $gallery_images)) : [];
        ...
        $this->selected_attributes = $selected_attributes; // Assign selected attributes from form
    ...
    protected function create_product($product_number) {
        ...
        // Handle stock management (if the checkbox was selected)
        ...
        if ($this->digital_product) {
            $product->set_virtual(true);
            $this->logger->log_info("Product $product_number: Set as virtual product.");
        }

        // Handle featured product
        if ($this->featured_product) {
            $product->set_featured(true);
            $this->logger->log_info("Product $product_number: Set as featured product.");
        }
        ...
        if (!empty($this->featured_images)) {
            ...
            $this->logger->log_info("Product $product_number: Featured image set.");
        }
        ...
        if (!empty($this->product_categories)) {
            $selected_category_ids = $this->select_random_categories();
            $product->set_category_ids($selected_category_ids);
            $this->logger->log_info("Product $product_number: Categories assigned.");
        }
    ...
    // Assign selected attributes to a variable product
    protected function assign_selected_attributes($product) {
        // Get only the attributes selected from the form
        ...
        if (!empty($selected_attributes)) {
            ...
            if (!empty($product_attributes)) {
                ...
                foreach ($variation_combinations as $variation_attributes) {
                    ...
                    // Optionally set variation images
                    if (!empty($this->variation_images)) {
                        $random_image_id = $this->variation_images[array_rand($this->variation_images)];
                        $variation->set_image_id($random_image_id);
                        $this->logger->log_info("Variation: Image set.");
                    }

                    // Optionally set variation description
                    if (!empty($this->variation_description)) {
                        $variation->set_description($this->variation_description);
                        $this->logger->log_info("Variation: Description set.");
                    }
